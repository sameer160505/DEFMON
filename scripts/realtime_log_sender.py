...
class RealtimeLogSender:
    def __init__(self, api_base: str, sender_id: Optional[str] = None, sender_key: Optional[str] = None):
        self.api_base = api_base.rstrip("/")
        self.sender_id = sender_id
        self.sender_key = sender_key
        self.session = requests.Session()

    def create_sender(self, admin_token: str, sender_name: str = "realtime-logger") -> tuple[str, str]:
    ...
    def generate_log_line(self, is_malicious: bool = False) -> str:
        ...
        log_line = f'{ip} - - [{ts_str}] "{method} {uri} {protocol}" {status} {bytes_sent} "{referrer}" "{user_agent}"'
        return log_line
    ...
    def send_logs(self, batch_size: int = 10, malicious_ratio: float = 0.3) -> dict:
        ...
        for attempt in range(retries):
            try:
                ...
                malicious_count = sum(1 for line in batch if any(uri in line for uri in MALICIOUS_URIS))
                logger.info(
                    f"✅ Sent batch: {len(batch)} logs "
                    f"({malicious_count} malicious, {len(batch) - malicious_count} normal) | "
                    f"Ingested: {result.get('accepted_lines', 0)}, Alerts: {result.get('generated_alerts', 0)}"
                )
                return result
            except Exception as e:
                if attempt < retries - 1:
                    logger.warning(f"⚠️  Send failed (attempt {attempt + 1}/{retries}): {type(e).__name__}. Retrying...")
                    time.sleep(2)
                else:
...
def main():
    ...
    parser.add_argument(
        "--sender-id",
        help="Existing sender ID (if not provided, will create new sender)"
    )
    parser.add_argument(
        "--sender-key",
        help="Existing sender API key (required if --sender-id is provided)"
    )
    parser.add_argument(
        "--batch-size",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Run for N seconds (default: 0 = indefinitely)"
    )
