...
async def get_metrics(
    ...
    mttd_values = [
        max(
            (a.created_at.replace(tzinfo=None) - a.timestamp.replace(tzinfo=None)).total_seconds(),
            0,
        )
        for a in alerts
        if a.created_at and a.timestamp
    ]
    ...
    mttr_values = [
        max(
            (i.closed_at.replace(tzinfo=None) - i.created_at.replace(tzinfo=None)).total_seconds(),
            0,
        )
